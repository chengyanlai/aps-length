...
def count_main_text_words_wordcount(tex_lines, opts):
    ...
    # this does not have a defined encoding in latex, so just read it as bytes
    wordcount_log = io.open('wordcount.log','rb').readlines()

    rm_files = glob.glob('wordcount*') + glob.glob('tmp_tex_file*')
    for file in rm_files:
        os.unlink(file)

    wordcount = len([line for line in wordcount_log if b'3.08633' in line or b'3.08635' in line])
    return wordcount
...
def count_figures_words(detex_lines, tex_lines, opts):
    ...
    for fig_i, fig_line in enumerate(fig_lines):
        ...
        this_fig_lines = [ (i, line) for i, line in enumerate(tex_lines) if filename in re.split('[{}]',line) and
